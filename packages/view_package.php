...
if (isset($_GET['id']) && $_GET['id'] > 0) {
    $productQry = $conn->query("SELECT p.*, v.agency_name as vendor, c.name as `category` FROM `package_list` p INNER JOIN agency_list v ON p.agency_id = v.id INNER JOIN category_list c ON p.category_id = c.id WHERE p.delete_flag = 0 AND p.id = '{$_GET['id']}'");
    ...
    if ($productQry->num_rows > 0) {
        ...
        $roomQry = $conn->query("SELECT 
                                ...
                                WHERE r.delete_flag = 0 
                                    ...
                                    AND r.package_category = '{$category}'");
                ...
                <div class="row">
                                ...
                                <?php foreach ($otherPackages as $otherPackage): ?>
                                    <div class="other-card">
                                        ...
                                        <div class="other-card-body">
                                            ...
                                            <a href="./?page=packages/view_package&id=<?= $otherPackage['id'] ?>" class="view-details-btn">View Details</a>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                    ...
                    <div class="col-lg-8 col-md-7 col-sm-12">
                        ...
                        <div class="row align-items-end room-selection">
                            ...
                            <!--<div class="col-md-3 form-group">
                                <a href="./?page=bookings/cart" class="btn btn-success btn-flat" id="checkoutBtn" disabled><i class="fa fa-check"></i> Book</a>
                            </div>-->
                        </div>
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp.status == 'success') {
                    ...
                    // Redirect to the bookings/cart page upon success
                    window.location.href = "./?page=bookings/cart";
                } else if (!!resp.msg) {
                    el.text(resp.msg);
                    $('#msg').append(el);
                    el.show('slow');
