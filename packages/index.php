                    ...
                    <div class="container-fluid">
                        ...
                        <div class="row" id="package_list" style="display: flex; flex-wrap: wrap;">
                            ...
                            if(isset($_GET['search']) && !empty($_GET['search'])){
                                $swhere .= " and (p.name LIKE '%{$_GET['search']}%' or p.description LIKE '%{$_GET['search']}%' or c.name LIKE '%{$_GET['search']}%' or v.agency_name LIKE '%{$_GET['search']}%') ";
                            }
                            $products = $conn->query("SELECT p.*, v.agency_name as vendor, c.name as `category` FROM `package_list` p inner join agency_list v on p.agency_id = v.id inner join category_list c on p.category_id = c.id where p.delete_flag = 0 and p.`status` =1 {$swhere} order by RAND()");
...
<script>
    $(function(){
        ...
        $('.cat_item').change(function(){
            ...
            location.href="./?page=packages&cids="+(ids.join(","))
        })
    ...
    $(function(){
        $('#categoryDropdown').change(function(){
            ...
            location.href = "./?page=packages&cids=" + selectedCategory;
        });
