...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT o.*, c.code as ccode, CONCAT(c.lastname, ', ', c.firstname, ' ', COALESCE(c.middlename, '')) as client, v.agency_name, v.code as vcode
                        FROM `booked_packages_list` o
                        INNER JOIN traveler_list c ON o.traveler_id = c.id
                        INNER JOIN agency_list v ON o.agency_id = v.id
                        WHERE o.id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
...
<script>
    $(function(){
        $('#update_status').click(function(){
            uni_modal_second("Update Order Status - <b><?= isset($code) ? $code : '' ?></b>","bookings/update_status.php?id=<?= isset($id) ? $id : '' ?>")
        });

        $('#update_payment_status').click(function(){
            uni_modal_second("Update Payment Status - <b><?= isset($code) ? $code : '' ?></b>", "bookings/update_payment_status.php?id=<?= isset($id) ? $id : '' ?>");
        });
    });
</script>
