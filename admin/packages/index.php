        ...
        <div class="container-fluid">
			<table class="table table-bordered table-stripped">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT p.*,v.code, v.agency_name as `vendor` from `package_list` p inner join agency_list v on p.agency_id = v.id where p.delete_flag = 0 order by p.`name` asc ");
...
<script>
	$(document).ready(function(){
		$('#create_new').click(function(){
			uni_modal('Add New Product',"packages/manage_product.php",'large')
		})
		$('.view_data').click(function(){
			uni_modal('View Product Details',"packages/view_package.php?id="+$(this).attr('data-id'),'large')
		})
