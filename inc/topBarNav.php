...
<nav class="w-100 px-2 py-1 position-fixed top-0 bg-dark text-light" id="login-nav">
  <div class="d-flex justify-content-between w-100">
    ...
    <div>
      <?php if ($_settings->userdata('id') > 0 && $_settings->userdata('login_type') == 3) : ?>
        <div class="dropdown">
          <a href="javascript:void(0)" class="dropdown-toggle text-reset text-decoration-none" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            <span class="mx-2"><img src="<?= validate_image($_settings->userdata('avatar')) ?>" class="img-thumbnail rounded-circle" alt="User Avatar" id="client-img-avatar"> <span class="mx-2"><?= !empty($_settings->userdata('firstname')) ? $_settings->userdata('firstname') : $_settings->userdata('email') ?></span>
          </a>
      ...
      <?php else : ?>
        <a href="./login.php" class="mx-2 text-light text-decoration-none font-weight-bolder" style="font-size: 0.8rem;">Customer Login</a> |
        <a href="./vendor" class="mx-2 text-light text-decoration-none font-weight-bolder" style="font-size: 0.8rem;">Resort Admin Login</a> |
        <a href="./admin" class="mx-2 text-light text-decoration-none font-weight-bolder" style="font-size: 0.8rem;">Superadmin Login</a>
      <?php endif; ?>
    </div>
  </div>
</nav>
      ...
      <ul class="navbar-nav ml-auto">
        ...
        <?php if ($_settings->userdata('id') > 0 && $_settings->userdata('login_type') == 3) : ?>
          <li class="nav-item">
            ...
            ?>
            <a href="./?page=orders/cart" class="nav-link <?= isset($page) && $page == 'orders/cart' ? "active" : "" ?>"><span class="badge badge-secondary rounded-circle"><!--<?= format_num($cart_count) ?>--></span> Checkout</a>
          </li>
          <li class="nav-item">
            <a href="./?page=orders/my_orders" class="nav-link <?= isset($page) && $page == 'orders/my_orders' ? "active" : "" ?>">My Bookings</a>
          </li>
        <?php endif; ?>
      </ul>
    </div>
